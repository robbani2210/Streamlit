...
# 1. Create button
if(st.button("Kelas streamlit")):
	st.text("Welcome To Playlist Belajar Fundamental Streamlit")

# 2. Create Checkbox
if st.checkbox("Show/Hide"):
	st.text("Hai Sahabat Kelas, Salam Keep Learning Temen temen")

# 3. Create radio button
status = st.radio("Pilih Playlist: ", ('NLP', 'Streamlit', 'Flask'))
st.write("Sahabat Kelas Siap belajar:", status)

# 4. Create Selection box
domain = st.selectbox("Sahabat Kelas Siap belajar: ",
					['NLP', 'Streamlit', 'Flask', 'DS'])
# print(Menapilkan)
st.write("Sahabat Kelas  Siap belajar: ", domain)

# 5. Create Multiselect
domain = st.multiselect("Sahabat Kelas Siap belajar: ", 
	['NLP', 'Streamlit', 'Flask', 'DS'])
# print(Menapilkan)
st.write("Sahabat Saat ini sedang belajar", len(domain), 'Playlist')
